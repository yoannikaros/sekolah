...
import '../admin/task_form_screen.dart';

class TeacherTaskManagementScreen extends StatefulWidget {
  const TeacherTaskManagementScreen({super.key});

  @override
  State<TeacherTaskManagementScreen> createState() => _TeacherTaskManagementScreenState();
}

class _TeacherTaskManagementScreenState extends State<TeacherTaskManagementScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (teacher == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Error: Teacher not found'),
      ...
      // Get class codes for teacher's school
      ...
      // Get all tasks and filter by teacher's subjects
      final allTasks = await _adminService.getAllTasks();
      final teacherTasks = allTasks.where((task) => 
        teacherSubjects.any((subject) => subject.name == task.mataPelajaran) &&
        schoolClassCodes.any((classCode) => classCode.code == task.kodeKelas)
      ).toList();
  ...
  List<AdminTask> get _filteredTasks {
    ...
    if (_selectedSubject.isNotEmpty) {
      filtered = filtered.where((task) => task.mataPelajaran == _selectedSubject).toList();
    }
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: [
                    ...
                    ..._teacherSubjects.map((subject) => DropdownMenuItem<String>(
                      value: subject.name,
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: [
                    ...
                    ..._schoolClassCodes.map((classCode) => DropdownMenuItem<String>(
                    ...
                    ? Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'Tugas akan muncul sesuai mata pelajaran yang Anda ajar',
                              style: TextStyle(
                                fontSize: 14,
